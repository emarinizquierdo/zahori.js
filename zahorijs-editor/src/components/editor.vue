<template>
  <transition name="modal uts-modal-editor" v-if="show">
    <div class="modal-mask">
      <div class="modal-wrapper">
        <div class="modal-container uts-modal-editor-container">
        ...
        <div class="uts page-header">
            ...
            <div class="uts buttons-group ng-scope">
                <button type="button" class="uts uts-btn btn-success" @click="$emit('close')">Cancel</button>
                <button v-if="!editing" type="button" class="uts uts-btn btn-info" @click="_addStep" :disabled="!step.title.es_ES || !step.title.en_US">Add</button>
                <button v-if="editing" type="button" class="uts uts-btn btn-info" @click="_saveStep" :disabled="!step.title.es_ES || !step.title.en_US">Save</button>
            </div>
        </div>
        ...
        <div id="uts-modal-editor-content" class="uts-modal-editor-content">
            ...
            <div class="uts uts-jumbotron">
                ...
                <div class="uts checkbox">
                    <div class="input-group">
                      <input id="showDOMSelectorCheckbox" type="checkbox" v-model="showDomSelector">
                      <label for="showDomSelectorCheckbox" >Change calculated DOM Selector</label>
                    </div>
                    <label v-if="showDomSelector" class="uts-warning">ONLY DO IT IF YOU KNOW EXACTLY THE DOM SELECTOR</label>
                    <label v-if="showDomSelector">DOM String Selector</label>
                    <input id="DOMSelector" v-if="showDomSelector" type="text" class="uts form-control" v-model="step.selector">
                </div>
            </div>
            ...
            <div class="uts uts-jumbotron">
                ...
                <input id="es_ES" placeholder="Give me a title" type="text" class="uts form-control" v-model="step.title.es_ES">
                <p></p>
                <label>Title (en_US)</label>
                <input id="en_US" placeholder="Give me a title" type="text" class="uts form-control" v-model="step.title.en_US">
                <p></p>
            </div>
          ...
          -->
            ...
            <div class="uts uts-jumbotron">
                ...
                <textarea id="description_es_ES" placeholder="Give me a description" class="uts" v-model="step.description.es_ES" ></textarea>
                <p></p>
                <label>Description (en_US)</label>
                <div id="medium_editor_toolbar_02"></div>
                <textarea id="description_en_US" placeholder="Give me a description" class="uts" v-model="step.description.en_US"></textarea>
                <p></p>
            </div>
...
export default {
  ...
  methods:{
    ...
    edit(stepIndex){
      ...
      this.backupStep = JSON.parse(JSON.stringify(this.$store.state.steps[stepIndex]));
      this.step = this.backupStep;
      this.open();
    },
    _saveStep(){

      store.commit('editStep',
